...
- Decision logic: Confirm underlay reachability and peer AS/role; enable the intended address family; attach import/export policy or next-hop handling only when a confirmed route-control outcome or peer constraint requires it; inspect peer and received/advertised routes; then execute the required path/failure test.
...
- Implementation order: Underlay; session; address family; selected policy/next-hop features only when required; route inspection; endpoint/path test; controlled fault and recovery.
